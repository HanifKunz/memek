...
const starts = async (dogexeon = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('Hanif Bot', {
		font: 'Standard',
	...
	console.log(color('\n> YT CHANNEL: Hanif ','silver'))
console.log(color('> GITHUB: DGHanif ','silver'))
console.log(color('> WA NUMBER: +15856056661 ','silver'))
console.log(color('  Hanif Bot Inc. 2021','mediumseagreen'))
	dogexeon.browserDescription = [ 'Subscribe Hanif', 'ubuntu', '3.0' ]
...
dogexeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "15856056661@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Hanif Bot`, img,but = [{buttonId:`getdescgc`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `Hanif Bot`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `GOODBYE !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
dogexeon.on('group-update', async (anu) => {
    ...
    dogexeon.on('chat-update', async (message) => {
        ...
        ownerNumber = ["15856056661@s.whatsapp.net","15856056661@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "15856056661@s.whatsapp.net"
